...
import { categoryShare, products, recentInvoices, salesTrend } from "@/lib/mock-data";
...
function Dashboard() {
  ...
  const expiringSoon = products.filter((p) => new Date(p.expiry) < new Date("2026-06-01"));

  return (
    <div className="space-y-6">
      {/* Stats */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="مبيعات اليوم" value="8,420 ر.س" delta={12.4} icon={DollarSign} tone="success" />
        <StatCard label="مبيعات الشهر" value="184,250 ر.س" delta={8.1} icon={TrendingUp} tone="primary" />
        <StatCard label="فواتير اليوم" value="47 فاتورة" delta={-3.2} icon={Receipt} tone="primary" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="animate-in-up rounded-2xl border border-border bg-card p-5 shadow-soft">
          <h3 className="font-bold">توزيع المبيعات حسب الفئة</h3>
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/40 text-muted-foreground">
                <tr>
                  ...
                  <th className="text-right font-medium px-5 py-3">نوع الدفع</th>
              ...
              <tbody>
                {recentInvoices.map((inv) => (
                  <tr key={inv.id} className="border-t border-border hover:bg-muted/30 transition">
                    <td className="px-5 py-3 font-semibold text-primary">{inv.id}</td>
                    <td className="px-5 py-3">{inv.customer}</td>
                    <td className="px-5 py-3 font-semibold">{inv.total.toLocaleString()} ر.س</td>
                    <td className="px-5 py-3">
                      <span className="rounded-md bg-accent px-2 py-1 text-xs">{inv.type}</span>
                    </td>
                    <td className="px-5 py-3 text-muted-foreground">{inv.time}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="space-y-4">
          ...
          <div className="animate-in-up rounded-2xl border border-border bg-gradient-primary text-primary-foreground p-5 shadow-glow">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="font-bold">المنتج الأكثر مبيعاً</span>
            </div>
            <div className="mt-2 text-lg font-extrabold">أوكسي تتراسيكلين 20%</div>
            <div className="text-sm opacity-90">132 وحدة هذا الشهر</div>
          </div>
        </div>
      </div>
    </div>
