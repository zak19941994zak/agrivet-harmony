...
import { Loader2, Plus, Search, Trash2, Pencil, Truck } from "lucide-react";
...
type Supplier = { id: string; name: string; phone: string | null; address: string | null; balance: number; notes: string | null };
...
function SuppliersPage() {
  ...
  useEffect(() => {
    const ch = supabase.channel("suppliers-rt")
      .on("postgres_changes", { event: "*", schema: "public", table: "suppliers" }, () => qc.invalidateQueries({ queryKey: ["suppliers"] }))
      .subscribe();
    return () => void supabase.removeChannel(ch);
  }, [qc]);

  const rows = useMemo(() => suppliers.filter((c) => !q || c.name.includes(q) || (c.phone ?? "").includes(q)), [suppliers, q]);
  const totalOwed = suppliers.reduce((s, c) => s + Number(c.balance), 0);
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-2xl border border-border bg-card p-5 shadow-soft">
          <div className="text-xs text-muted-foreground">إجمالي الموردين</div>
          <div className="text-2xl font-extrabold mt-1">{suppliers.length}</div>
          <Truck className="h-5 w-5 text-primary/40 mt-2" />
        </div>
        <div className="rounded-2xl border border-border bg-card p-5 shadow-soft">
          <div className="text-xs text-muted-foreground">مستحقات للموردين</div>
          <div className="text-2xl font-extrabold mt-1 text-warning">{totalOwed.toLocaleString()} ر.س</div>
        </div>
        <div className="rounded-2xl border border-border bg-card p-5 shadow-soft">
          <div className="text-xs text-muted-foreground">موردين بأرصدة</div>
          <div className="text-2xl font-extrabold mt-1">{suppliers.filter((s) => Number(s.balance) > 0).length}</div>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-soft">
        ...
        rows.length === 0 ? <div className="text-center py-20 text-muted-foreground text-sm">لا يوجد موردين</div> :
      ...
      {open && <SupplierDialog initial={editing} onClose={() => setOpen(false)} />}
    </div>
  );
}

function SupplierDialog({ initial, onClose }: { initial: Supplier | null; onClose: () => void }) {
  const [form, setForm] = useState({
    name: initial?.name ?? "", phone: initial?.phone ?? "", address: initial?.address ?? "",
    notes: initial?.notes ?? "", balance: initial?.balance ?? 0,
  });
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-background/70 backdrop-blur-sm p-4 animate-in-up">
      <div className="w-full max-w-xl rounded-2xl border border-border bg-card shadow-elegant">
        <div className="p-5 border-b border-border flex items-center justify-between">
          <h3 className="font-bold text-lg">{initial ? "تعديل مورد" : "مورد جديد"}</h3>
        ...
        <form onSubmit={submit} className="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Field label="الرصيد المستحق"><input type="number" step="0.01" value={form.balance} onChange={(e) => setForm({ ...form, balance: +e.target.value })} className={inp} /></Field>
